...
const TableGray = ({title, source, displayHeaders}) => {

    const headers = displayHeaders ? Object.keys(displayHeaders) : Object.keys(source[0]);
    ...
    useEffect(() => {
        ...
        refactor(source).then(res=>console.log(res));

        // if(displayHeaders){
        //     setSourcy(sourceRefactor)
        // }
        // else {
        //     setSourcy(source)
        // } 
        ...
        source.map((row) => {
            ...
            headers.map(head => {
                if( typeof displayHeaders[head] == 'string' ) { 
                    newObj.push(displayHeaders[head])
                }
    ...
    const sourceRefactor = async (displayHeaders, headers, source) => {
        const newArr = [];
        
        source.map((row) => {
            const newObj = {};
            newArr.push(headers.map((head)=>{
                if( typeof displayHeaders[head] === 'string' ) {
                    newObj[head] = row[head];
                }
                else if ( Array.isArray(displayHeaders[head]) ) {
                    let comp = '';
                    displayHeaders[head].map(meow => (
                        comp += meow +' '                        
                    ))
                }
